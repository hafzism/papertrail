...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ next?: string; error?: string }> }) {
  ...
  if (config === null) {
    return (
        <main className="pt-public-page mx-auto flex min-h-screen w-full max-w-xl items-center px-5 py-12">
        <section className="w-full rounded-none border border-[var(--pt-rule)] bg-white p-7 shadow-none" aria-labelledby="signin-title">
          <p className="pt-eyebrow">Setup required</p>
  ...
  return (
    <main className="pt-public-page mx-auto flex min-h-screen w-full max-w-xl items-center px-5 py-12">
      <section className="w-full rounded-none border border-[var(--pt-rule)] bg-white p-7 shadow-none" aria-labelledby="signin-title">
        <p className="pt-eyebrow">Secure sign-in</p>
