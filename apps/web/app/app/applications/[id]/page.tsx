...
export default async function ApplicationPage({ params }: ApplicationPageProps) {
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-3xl px-5 py-10 sm:px-8">
      ...
      <p className="mt-8 text-sm font-semibold uppercase tracking-[0.15em] text-[var(--navy)]">Private application</p>
      <h1 className="mt-3 text-4xl font-bold tracking-tight">{application.title}</h1>
      <p className="mt-4 leading-7 text-[var(--slate)]">Lifecycle: {application.lifecycle_state.replaceAll("_", " ")} · Readiness: {application.readiness_state.replaceAll("_", " ")}</p>
      {applicationCycle ? <p className="mt-2 text-sm text-[var(--slate)]">Directory connection: {applicationInstitution?.name ?? "Institution"} · {applicationProgram?.name ?? "Program"} · {applicationCycle.cycle_label}. {application.monitoring_enabled ? "Future-update preference recorded. If a moderator-enabled public source is revised and approved, PaperTrail will send a private review notification; it does not re-evaluate your requirements automatically." : "No future-update preference recorded."}</p> : <p className="mt-2 text-sm text-[var(--slate)]">Private one-off draft: no shared program/cycle is linked.</p>}
